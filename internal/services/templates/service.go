...
func GetAllTemplates(req *GetAllTemplatesRequest, store *Store) (*definition.Response[[]definition.Template], error) {
	...
	if err != nil {
		utils.LogError(err, "get all templates", definition.ErrInternalServer, definition.AppError_Error_Severity)
		return nil, err
	}

	numberOfRows, err := store.CountAllTemplates(req.Keyword, req.CategoryId, req.Slots)
	if err != nil {
		utils.LogError(err, "count all templates", definition.ErrInternalServer, definition.AppError_Error_Severity)
		return nil, err
	}
